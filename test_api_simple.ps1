...
try {
    $response = Invoke-WebRequest -Uri "$baseUrl/auth/login/" -Method POST -Headers @{"Content-Type"="application/json"} -Body $loginBody -UseBasicParsing
...
if ($accessToken) {
    ...
    try {
        $response = Invoke-WebRequest -Uri "$baseUrl/auth/profile/" -Method GET -Headers @{"Authorization"="Bearer $accessToken"; "Content-Type"="application/json"} -UseBasicParsing
        $data = $response.Content | ConvertFrom-Json
...
try {
    $response = Invoke-WebRequest -Uri "$baseUrl/auth/register/" -Method POST -Headers @{"Content-Type"="application/json"} -Body $regBody -UseBasicParsing
    $data = $response.Content | ConvertFrom-Json
...
try {
    $response = Invoke-WebRequest -Uri "$baseUrl/auth/forgot-password/" -Method POST -Headers @{"Content-Type"="application/json"} -Body $forgotBody -UseBasicParsing
    $data = $response.Content | ConvertFrom-Json
